...
13. **Documentación Obligatoria y Continua:** Cada vez que se programe o despliegue una nueva sección, módulo o feature en Brokers Connector V2, es **OBLIGATORIO** ejecutar las siguientes tres acciones antes de considerar la entrega como completada: (a) actualizar `Manual_Administrador.html` con la nueva funcionalidad desde la perspectiva del administrador; (b) actualizar `Manual_Usuario.html` con el impacto en el flujo del agente inmobiliario en lenguaje operativo B2B; (c) indexar el nuevo recurso en el Hub Central (`/reportes/index.html`) con título, descripción y fecha. **Sin documentación actualizada = feature NO entregado.** Esta regla aplica sin excepción a cualquier IA, desarrollador o arquitecto que opere sobre este sistema.
14. **Invariabilidad y Secuencia del Ecosistema Visual (UI/UX Continuity):** NINGUNA vista, pestaña, módulo o pantalla del ecosistema V2 puede desviarse del estándar visual establecido por el módulo de referencia (`ai-hub`). La homologación es **TOTAL e IRREVOCABLE** e incluye: (a) **Nomenclatura exacta de controles de navegación**: los textos de botones son contratos de marca — `"Panel de Control"`, `"Broker Brain IA"`, `"Cerrar Sesión"` son los únicos nombres válidos; cero traducciones libres ni abreviaciones (`"Panel"`, `"Salir"` están **PROHIBIDOS**); (b) **Estructura DOM del header**: el menú móvil (`<nav class="v2-mobile-menu">`) debe vivir **dentro** del `<header>`, nunca como elemento externo; (c) **Sistema de clases compartidas**: todo módulo V2 consume las clases de `v2.css` (`.v2-header`, `.v2-nav-link`, `.v2-mobile-menu`, `.v2-hamburger`, etc.) sin redefinirlas en su CSS local; (d) **URLs dinámicas obligatorias**: ningún enlace de navegación puede tener rutas hardcodeadas — todas se construyen desde `state.apiBase` resuelto del parámetro `?api=` inyectado por el BridgeController; (e) **Botón de retorno siempre operativo**: el enlace de la pantalla de error (`error-back-btn`) debe configurarse **antes** de cualquier `return` en el bootstrap del módulo, garantizando su funcionamiento incluso cuando el token de autenticación sea inválido o haya expirado. **Violación de cualquiera de estos incisos = entrega rechazada.**

15. **Gobernanza de Agentes Federados — Ley de Subordinación Absoluta al Kernel AURA:** Todo agente operativo federado bajo el ecosistema Brokers Connector V2 (incluyendo, sin limitarse a, `PulseScout IA`) carece por diseño de autonomía de conexión directa hacia cualquier proveedor de inteligencia artificial. **REGLA INMUTABLE:** Todo estímulo multimodal recibido por un agente federado — ya sea texto, audio transcrito, imagen codificada en Base64 o contexto documental — DEBE ser enrutado obligatoriamente a través del **Kernel AURA** de Brokers Connector antes de alcanzar cualquier backend de IA. El Kernel AURA es el ÚNICO ente del sistema autorizado para: (a) **Contabilizar** el consumo de tokens y cuotas en el ledger centralizado; (b) **Auditar** cada transacción de IA en las tablas `ai_conversations` y `ai_messages`; (c) **Registrar** el Tenant activo (`company_id`) en cada solicitud bajo Tenant Lock Absoluto; (d) **Enrutar en cascada** de forma dinámica — Capa A (ACADEP Soberano) → Capa B (proveedores comerciales) → Capa C (Mock de validación) — sin que el agente federado tenga visibilidad ni control sobre este mecanismo; (e) **Decretar el bypass de emergencia**: bajo el escenario de colapso de API Keys comerciales, AURA dictaminará de forma inmutable el desvío directo y exclusivo hacia el motor soberano de ACADEP Linux para garantizar la continuidad del negocio sin intervención humana. **Consecuencia de violación:** Cualquier agente que establezca conexión directa con un proveedor de IA, bypaseando el Kernel AURA, genera registros de consumo fantasma no auditables, viola el Tenant Lock y compromete la integridad del ledger de CAPEX. Dicho código debe ser revertido inmediatamente y el incidente registrado en el Audit Trail.
